...
for i, img_name in enumerate(progress_bar):
    ...
    for idx_bin, bin in enumerate(overlapping_bins):
        ...
        depth_layer = np.where(mask, depth_image_resized_norm, 0)
    ...
    for depth_layer in depth_layers:
        ...
        depth_layers_image.append([depth_layer, depth_layer_atts, layer])
    ...
    plot_intermediate_images(title, pil_img, sum_atts_resized, depth_image_resized, final_attention_map, final_attention_map_simple, final_att_thresholded, final_image_simple)

    # Let´s draw the depth_layers_image as a grid
    plot_depth_layers(title, depth_layers_image)

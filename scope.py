...
for i, img_name in enumerate(progress_bar):
    ...
    if (continue_reading):
        ...
        if (line):
            ...
            if (img_name in line):
                ...
                # get the odAP50 from the line
                odAP50[i] = float(line.split(",")[3])
                local_odAP50 = odAP50[i]
                odAP50_now = odAP50[:i+1].sum() / (i+1)

                # show the image name in the progress bar without breaking the tqdm
                progress_bar.set_description(f"Processed {img_name} // Image corloc: {local_corloc:.2f} // Corloc now: {corloc_now:.2f} // odAP50: {local_odAP50:.2f} // odAP50 now: {odAP50_now:.2f}")                
    ...
    #for i in range(6):
    #    axs[i // 3, i % 3].imshow(atts[i])
    #    axs[i // 3, i % 3].set_title(f'Attention {i+1}')
    ...
    local_odAP50 = odap50_no_score(ground_truth_as_list, final_boxes, iou_threshold=0.5)

    # Storing image-corloc to obtain final dataset-corloc later
    corloc[i] = local_corloc
    odAP50[i] = local_odAP50
    ...
    progress_bar.set_description(
        f"Processed {img_name} // Image corloc: {local_corloc:.2f} // Corloc now: {corloc_now:.2f} // odAP50: {local_odAP50:.2f} // odAP50 now: {odAP50_now:.2f}"
    )
    ...
    title = f"Image: {img_name} // Corloc: {corloc[i]:.2f} // odAP50: {odAP50[i]:.2f}"
    ...
    # Let´s save the thresholded_layer from depth_layers_image as individual images
    for idx, (depth_layer, depth_layer_atts, thresholded_layer) in enumerate(depth_layers_image):

        filename = f"analysis/{timestamp}_images/{img_name}_thresholded_layer_{idx}.png"
        print(filename)

        # Let´s save with plt.imshow
        fig = plt.figure()
        plt.imshow(thresholded_layer)
        plt.axis('off')
        plt.savefig(filename)
        plt.close(fig)

        filename = f"analysis/{timestamp}_images/{img_name}_depth_layer_{idx}.png"
        print(filename)

        # Let´s save with plt.imshow
        fig = plt.figure()
        plt.imshow(depth_layer)
        plt.axis('off')
        plt.savefig(filename)
        plt.close(fig)
    ...
    # write the results to the file: i, img_name, corloc, odap50 ious, predicted_boxes, ground_truth
    results_file.write(f"{i},{img_name},{corloc[i]},{odAP50[i]},{ious_b64},{predicted_boxes_b64},{ground_truth_b64}\n")
...
# Let's calculate odAP50 as the percentage of images with at least one IoU > 0.5
odAP50 = odAP50.sum() / len(images)

print("-------------------")
print("Final CorLoc: ", corloc)
print("Final odAP50: ", odAP50)
print("-------------------")
